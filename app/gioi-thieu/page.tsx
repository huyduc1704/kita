...
import { getSystemSetting, SystemSetting, getAboutPosts, NewsItem } from '@/utils/api';
...
export default function AboutPage() {
  ...
  // Lọc bài viết
  const visionPost = aboutPosts.find(p => p.slug.includes('tam-nhin') || p.title.toLowerCase().includes('tầm nhìn'));
  const missionPost = aboutPosts.find(p => p.slug.includes('su-menh') || p.title.toLowerCase().includes('sứ mệnh'));
  const mainPost = aboutPosts.find(p => p !== visionPost && p !== missionPost);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            <h2 className="text-2xl font-bold font-serif text-zinc-900 leading-tight uppercase">
              {mainPost ? mainPost.title : "KIẾN TẠO KHÔNG GIAN SỐNG HOÀN MỸ"}
            </h2>
          ...
          <div className="lg:col-span-5 relative group">
            ...
            <div className="relative bg-white rounded-lg overflow-hidden shadow-xl border border-zinc-200">
              <img
                src={mainPost?.image && mainPost.image !== 'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?auto=format&fit=crop&w=800&q=80' ? mainPost.image : "https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=800&q=80"}

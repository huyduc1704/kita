...
export default function ProjectDetailPage() {
  ...
  if (loading) {
    return <div className="text-center py-20 text-zinc-500 font-sans">Đang tải chi tiết dự án...</div>;
  }
  ...
  if (!currentProject) {
    return <div className="text-center py-20 text-zinc-500 font-sans">Không tìm thấy dữ liệu dự án.</div>;
  }

  // Related projects: same category, excluding current project, capped at 3
  const relatedProjects = allProjects
    .filter((p) => p.category === currentProject.category && p.slug !== currentProject.slug)
    .slice(0, 3);

  // Friendly Category name mapper
  const categoryNames: Record<string, string> = {
    'nha-pho': 'Nhà phố',
    'mai-nhat': 'Nhà mái Nhật',
    'mai-thai': 'Nhà mái Thái',
    'biet-thu': 'Biệt thự',
    'nha-vuon': 'Nhà vườn',
    'noi-that': 'Nội thất',
  };
    ...
    <article className="py-10 bg-zinc-50 min-h-screen font-sans">
      <div className="max-w-[1280px] mx-auto px-4">
        ...
        <div className="flex items-center gap-1 text-zinc-500 text-xs font-semibold mb-6">
          ...
          <span>/</span>
          <Link href="/du-an" className="hover:text-[#f39221] transition-colors">Dự án</Link>
          <span>/</span>
          <span className="text-zinc-800">{categoryNames[currentProject.category] || 'Chi tiết'}</span>
        </div>
        ...
        <div className="flex flex-col gap-3 mb-8">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-[#007ba1] bg-sky-500/10 px-2.5 py-1 rounded">
              {categoryNames[currentProject.category] || 'Kiến trúc'}
            </span>
          </div>
            ...
            <div className="text-zinc-700 text-sm md:text-base font-light leading-relaxed flex flex-col gap-6 text-justify">
              ...
              {currentProject.description ? (
                ...
                <div className="flex flex-col gap-4">
                  <p>
                    Công trình được thiết kế và thi công bởi đội ngũ kiến trúc sư, kỹ sư tâm huyết và giàu kinh nghiệm của <strong>Gamma Home</strong>. Chúng tôi tự hào mang tới giải pháp kiến trúc ấn tượng, hài hòa phong thủy và tối ưu công năng tối đa.
                  </p>
                  <p>
                    Để đạt được chất lượng hoàn mỹ này, quy trình từ khảo sát hiện trạng đất đai, lên ý tưởng phối cảnh 3D mặt tiền cho tới triển khai toàn bộ hồ sơ bản vẽ kỹ thuật chi tiết đều tuân thủ các quy chuẩn xây dựng nghiêm ngặt nhất.
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {currentProject.gallery.map((imgUrl, index) => (
                    <div key={index} className="overflow-hidden rounded-lg bg-zinc-100 border border-zinc-200 shadow-sm group">
                      <img
                        src={imgUrl}
                        alt={`${currentProject.title} - Ảnh ${index + 1}`}
                ...
                >
                  ...
                  <div className="p-4 flex flex-col gap-2">
                    ...
                    <h4 className="font-semibold text-xs md:text-sm text-zinc-900 group-hover:text-[#f39221] transition-colors uppercase line-clamp-2 min-h-[40px] leading-snug">
                      <Link href={`/du-an/${proj.slug}`}>{proj.title}</Link>
                    </h4>
                    ...
                    <div className="flex justify-between items-center mt-2 pt-2 border-t border-zinc-100 text-xs">
                      <span className="text-[#f39221] font-bold hover:underline">
                        <Link href={`/du-an/${proj.slug}`}>Chi tiết &raquo;</Link>
                      </span>
                    </div>
                  </div>
                </div>

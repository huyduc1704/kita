export default function DoubleBanner({ config }: { config?: any }) {
  return (
    <section className="w-full grid grid-cols-1 md:grid-cols-2">
      ...
      <div className="relative h-[280px] md:h-[350px] overflow-hidden">
        ...
        <div 
          ...
          style={{ backgroundImage: `url('${config?.banner1?.imageUrl || '/kita/Kita-Home-22.jpg'}')` }} 
        />
        {/* Overlay */}
      ...
      <div className="relative h-[280px] md:h-[350px] overflow-hidden">
        ...
        <div 
          className="absolute inset-0 bg-cover bg-center" 
          style={{ backgroundImage: `url('${config?.banner2?.imageUrl || '/kita/Kita-Home-24.jpg'}')` }} 
        />
        {/* Overlay */}
        <div className="absolute inset-0 bg-black/60 z-10" />
        ...
        <div className="absolute inset-0 z-20 flex flex-col justify-center items-center text-center p-6 md:p-12 text-white">
          ...
          <p className="text-primary-light text-xs md:text-sm font-medium tracking-widest mt-4 uppercase">
            {config?.banner2?.subtitle || 'Khi Xây Nhà Trọn Gói'}
          </p>
        </div>
      </div>
    </section>
